...
class DataLeakageAndroidSourceAndSinkManager(sasFilePath: String) extends DefaultAndroidSourceAndSinkManager(sasFilePath){
  ...
  private def sensitiveData: ISet[String] = Set("[B")

  
 /*  override def isSourceMethod (apk: ApkGlobal, calleeSig: Signature ): Boolean = {
  
      
   // false
      calleeSig.toString().contains("Landroid/bluetooth/BluetoothDevice;.createBond:()Z") || super.isSinkMethod(apk, calleeSig)
  }*/
  
  
  override def isCallbackSource(apk: ApkGlobal, sig: Signature, pos: Int): Boolean = {
/*   println("callback:"+apk.model.getCallbackMethods)
    println("sig:"+sig.toString())*/
   ...
   */
    ...
    if(apk.model.getCallbackMethods.contains(sig)&&sig.getSubSignature.contains("onLeScan")){
